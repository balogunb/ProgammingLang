...
Var* createE(char * name, char * expS, SymNode* table){
	...
	if(expS[0] != '(' && !isdigit(expS[0]) ){
		SymNode* existing = findSymNode(table,expS);
		Var* k = createVariable();
			k->name = name;
		 k->exp = existing->var->exp;
		 // SymNode* new = createSymNode();
		 // setVar(new, k);
		 addToSymTable(table,k);
		 return k;

	}

	//create a variable with the given expression
	Var* foo = createVarWExp(name, expS);


	//if foo already exist in the table change the value 
	if(findSymNode(table,name) != 0){
		SymNode* currNode = findSymNode(table,name);
		setVar(currNode, foo);
	}
...
long int evalSubsetFunct(Var* ex,SymNode* table){
	...
	if(exp->symbol ){
		int x = strcmp(exp->symbol,"+");
		int y = strcmp(exp->symbol,"-");
		int z = strcmp(exp->symbol,"*");


		if(x   && y  &&z){
			Exp* temp2 = createExpNode();
			temp2->symbol = (char*)malloc(100*sizeof(char));
			strcpy(temp2->symbol,exp->symbol);
			return evalFunctHelper(temp2,table);
